...
async def wait_for_address(msg: types.Message, state: FSMContext):
    ...
    if matches_map := Database.get_matches_map(request, city_name):
        ...
        closest_addresses_names: list[str] = difflib.get_close_matches(
            request,
            list(
                map(
                    lambda address: (
                        (address.street or "") + " " + (address.number or "")
                    ),
                    addresses,
                )
            ),
        ...
        closest_maps = [
            Database.get_matches_map(address, city_name)
            for address in closest_addresses_names
        ]
        for address, map_obj in zip(closest_addresses_names, closest_maps):
            if map_obj:
                button = types.InlineKeyboardButton(
                    map_obj.name, callback_data=f"map_{map_obj.id}"
                )  # type: ignore
